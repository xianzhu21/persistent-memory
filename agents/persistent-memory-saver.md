...
- **Scenarios (when this runs well):**
  ...
  2. **Manual default — existing summaries only** (manual `/persistent-memory-save`, **`existing-summaries`**): **refresh-only set** — read **`.cursor/hooks/state/persistent-memory.json`** and include only `conversation_id`s whose **`lastTranscriptMtimeMs` is not null** (Stop hook sets it when cadence fires the save follow-up; proxy for “already went through hook save promotion”). Map those ids to parent `*.jsonl` under step‑1 roots. **Does not** list **`~/.cursor/persistent-memory/summaries/*.md`** (cheap at scale). Ids still **null** in hook state are **not** processed here; **default first-time** capture for those ids is Stop hook **`current-session`** unless the user uses **`explicit-transcript`** (scenario 2b).
  2b. **Manual — explicit transcript path** (manual `/persistent-memory-save` with an absolute **`*.jsonl`** path, often **`@/…/agent-transcripts/…/….jsonl`**): **`explicit-transcript`** — process **exactly that** parent transcript if it exists, lies under an **`agent-transcripts/`** tree, and is **not** under **`…/subagents/`**. **No** hook-state id filter and **no** requirement that the file live under this workspace’s step‑1 roots (the user may point at any Cursor project on disk). Enables **first-time** summary + gzip for a chat the Stop hook never promoted.
...
- **Scope:** Task prompt **`PERSISTENT_MEMORY_SCOPE`** is one of **`explicit-transcript`**, **`existing-summaries`**, or **`current-session`**. **Resolution order:**
  1. If the prompt sets **`PERSISTENT_MEMORY_SCOPE=explicit-transcript`**, use it (**required:** **`transcript_path=`** — see below). **Do not** override with hook token or manual default.
  2. Else if upstream includes **`PERSISTENT_MEMORY_TRIGGER=stop-hook`**, use **`current-session`**.
  3. Else (manual **`/persistent-memory-save`** without **`explicit-transcript`**) use **`existing-summaries`**.
  If the scope line is missing, infer: **`explicit-transcript`** only if **`transcript_path=`** is present **and** the path is absolute, ends with **`.jsonl`**, contains **`/agent-transcripts/`**, and does **not** contain **`/subagents/`** (parent chat intent); otherwise apply steps 2–3 above. Treat **`hook-state`** as a deprecated alias for **`existing-summaries`** (same rules).
- **`explicit-transcript` (required):** **`transcript_path=<absolute path>`** to a single parent **`*.jsonl`**. Strip a single leading **`@`** if present (Cursor attachment style). Path must be absolute; **do not** silently resolve cwd-relative paths. If **`transcript_path=`** is missing, empty, or invalid under **Discover transcripts → `explicit-transcript`**, respond with a brief error outcome and **do not** fall back to other scopes.
- **Optional hints (`current-session` only):** `conversation_id=<full id>` and/or `transcript_path=<absolute path to *.jsonl>` when the parent agent supplies them (must still lie under a step‑1 root for **`current-session`** — see **Current session resolution**).
...
1. **Resolve transcript root(s)**
   ...
   **Do not** only scan the folder-opened slug if a `*code-workspace/agent-transcripts` sibling exists for the same repo token.
   Union all discovered roots. Step 3 uses a **full tree walk** only for **`current-session`**; for **`existing-summaries`** or **`explicit-transcript`**, use **targeted paths only** (no full walk under `agent-transcripts/`).
...
3. **Discover transcripts to process**
   ...
   Run **one** branch below according to scope. Do **not** run the **`current-session`** tree walk when scope is **`existing-summaries`** or **`explicit-transcript`**.

   **`PERSISTENT_MEMORY_SCOPE=explicit-transcript` (manual path)** — **no** tree walk and **no** hook state read:
   1. Read **`transcript_path=`** from the Task prompt; strip optional leading **`@`**; trim whitespace. **Reject** if missing, empty, not absolute (e.g. must start with **`/`** on Unix), or does not end with **`.jsonl`**.
   2. The path must contain **`/agent-transcripts/`** as a substring (normalized absolute path, forward slashes). **Reject** if not (not a Cursor agent transcript location).
   3. **Subagent guard:** if the path contains **`/subagents/`** after **`agent-transcripts/`** (worker JSONL), respond with an error outcome — **do not** switch to index-only in this scope; the user asked for a **session** save. Point them at the **parent** **`{id}/{id}.jsonl`** instead.
   ...
   **Skip** when scope is **`explicit-transcript`** (candidate list is already the single path). After you have the **full** candidate set from the **`current-session`** tree walk above, determine **exactly one target transcript path** for this run (do not process others):
   ...
   - **`PERSISTENT_MEMORY_SCOPE=explicit-transcript`:** the list is **already** the single path from the **`explicit-transcript`** subsection—do **not** union with step‑1 discovery, hook state, or **`current-session`** resolution.
