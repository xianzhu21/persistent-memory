...
- **Multi-topic sessions (MUST):** One `conversation_id` can contain **several** substantive threads (e.g. Notion **Task A** then **Task B**, or Jira import then a different **`/van`**). This is **not** optional to capture.
  ...
  5. **Incremental merge:** When an existing summary is present, **re-scan the full transcript** on every write. If the old summary omitted a thread that exists in JSONL, **add** it (do not leave gaps). Only remove or annotate a thread when the transcript **supersedes** it (use merge rule: "—superseded by …").
