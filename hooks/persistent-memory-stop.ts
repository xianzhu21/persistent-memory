...
const FOLLOWUP_BODY =
  `Run the \`persistent-memory-save\` skill now. Use the \`persistent-memory-saver\` subagent for the full session summary flow. Use incremental transcript processing with index file \`~/.cursor/persistent-memory/incremental-index.json\`: only consider transcripts not in the index or transcripts whose mtime is newer than indexed mtimeMs. Follow \`agents/persistent-memory-saver.md\` for transcript roots (including folder vs \`*code-workspace\` project slugs) and for **subagent** transcript paths: JSONL under \`.../agent-transcripts/.../subagents/\` get **index-only** updates (no \`summaries/*.md\`, no gzip, no \`sessions.md\` row)—user memory is the **parent** chat, not the worker log. Have the subagent merge and write \`~/.cursor/persistent-memory/summaries/{conversation_id}.md\` for user sessions, archive to \`~/.cursor/persistent-memory/transcripts/{conversation_id}.jsonl.gz\`, and update \`sessions.md\` and the index as in the doc. Avoid generating temporary helper scripts (e.g. ad-hoc Python files); do the required work via direct file operations and minimal shell commands only. If no transcripts need updates or there is no substantive content, respond exactly: No session summary generated (no substantive content); index updated.`;
