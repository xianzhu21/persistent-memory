...
Each **non-duplicate** Stop (after `generation_id` dedupe and cadence updates) can remove **other** `conversations` entries whose **`lastTranscriptMtimeMs`** is older than **`N` days** (default **`N` = 60** when **`PERSISTENT_MEMORY_CONVERSATIONS_PRUNE_AFTER_DAYS`** is unset). Set **`PERSISTENT_MEMORY_CONVERSATIONS_PRUNE_AFTER_DAYS=0`** to **disable** pruning. That field is the **transcript `.jsonl` mtime** stored when cadence **last fired a follow-up** for that chat (same watermark as `hasTranscriptAdvanced`). Chats that **never** reached a hook trigger keep **`lastTranscriptMtimeMs` null** and are **not** pruned. Default manual save (**`existing-summaries`**) **only** includes `conversation_id`s whose **`lastTranscriptMtimeMs`** is **not null** (hook has promoted a save at least once). Ids still **null** are not covered by manual **`existing-summaries`**; they enter the refresh set only after the Stop hook **`current-session`** flow has run and cadence has set **`lastTranscriptMtimeMs`**. The **current** `conversation_id` key is never removed in that pass.
